...
LacunaWaX::Model::DBILogger - DBI-based output channel for Log::Dispatch.

=head1 SYNOPSIS

 my $dbh = DBI->connect("dbi:SQLite:dbname=/path/to/database/file.sqlite", q{}, q{} );

 my $dbi_output = LacunaWaX::Model::DBILogger->new(
  name        => 'dbi',
  min_level   => 'debug',
  dbh         => $dbh,
  table       => 'Logs',
...
LacunaWaX::Model::DBILogger does not require Log::Dispatch to be subclassed, but it 
