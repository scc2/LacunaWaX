...
This script's flow:
    ...
    - Attempt to create all tables/indexes/constraints needed for the app
        ...
        - create_tables_and_indexes() is doing this creation.
...
When setting up a new version that includes new tables/indexes/constraints:
    ...
    - Modify create_tables_and_indexes() so it calls your new sub.
...
$main_path =~ s{\\}{/}g;
say $ilog "Deploying database to $main_path";
my $main_dsn    = "DBI:SQLite:dbname=$main_path";
my $sql_options = {sqlite_unicode => 1, quote_names => 1};
my $main_schema = LacunaWaX::Model::Schema->connect($main_dsn, $sql_options) or die "no schema: $!";
...
say $ilog "Creating schema elements";
create_tables_and_indexes($main_schema);
...
### Set the DisplayVersion registry key to be equal to $LacunaWaX::VERSION so on 
### the next upgrade this script will know what had previously been installed.
...
sub create_tables_and_indexes {#{{{
    my $schema  = shift;
    my $dbh     = $schema->storage->dbh;

    say $ilog "-=-=-=-";
    say $ilog "Generating 1.0 Schema:";
    my $one_point_oh = tables_1point0();
    while(my($name,$stmt) = each %$one_point_oh) {
    ...
    my $one_point_one = tables_1point1();
    while(my($name,$stmt) = each %$one_point_one) {
        say $ilog "Attempting to create '$name':";
        try {
            $dbh->do($stmt);
        }
    ...
    my $one_point_ten = tables_1point10();
    while(my($name,$stmt) = each %$one_point_ten) {
        say $ilog "Attempting to create '$name':";
        try {
            $dbh->do($stmt);
        }
    ...
    if( $create_test_tables ) {
        ...
        my $test_tables = tables_testing();
        while(my($name,$stmt) = each %$test_tables) {
            try {
                $dbh->do($stmt);
            }
            catch {
...
CREATE TABLE BodyTypes (
  ...
  type_general varchar(16)
    ...
    $table_statements->{'BodyTypes_index_body_id'} = 'CREATE INDEX BodyTypes_body_id ON BodyTypes (body_id)';
    $table_statements->{'BodyTypes_index_type_general'} = 'CREATE INDEX BodyTypes_type_general ON BodyTypes (type_general)';
    $table_statements->{'BodyTypes_constraint_one_per_server'} = 'CREATE UNIQUE INDEX one_per_server ON BodyTypes (body_id, server_id)';
...
CREATE TABLE ServerAccounts (
  ...
  FOREIGN KEY(server_id) REFERENCES Servers(id)
    ...
    $table_statements->{'ServerAccounts_index_server_id'} = 'CREATE INDEX ServerAccounts_idx_server_id ON ServerAccounts (server_id)';
...
CREATE TABLE Servers (
  ...
  protocol varchar(8) DEFAULT 'http'
    ...
    $table_statements->{'Servers_constraint_unique_by_name'} = 'CREATE UNIQUE INDEX unique_by_name ON Servers (name)';
...
CREATE TABLE SpyTrainPrefs (
  ...
  train varchar(32)
    ...
    $table_statements->{'SpyTrainPrefs_index_spy_id'} = 'CREATE INDEX SpyTrainPrefs_spy_id ON SpyTrainPrefs (spy_id)';
    $table_statements->{'SpyTrainPrefs_index_train'} = 'CREATE INDEX SpyTrainPrefs_train ON SpyTrainPrefs (train)';
...
CREATE TABLE ArchMinPrefs (
  ...
  auto_search_for varchar(32)
    ...
    $table_statements->{'ArchMinPrefs_index_body_id'} = 'CREATE INDEX ArchMinPrefs_body_id ON ArchMinPrefs (body_id)';
    $table_statements->{'ArchMinPrefs_index_server_id'} = 'CREATE INDEX ArchMinPrefs_server_id ON ArchMinPrefs (server_id)';
    $table_statements->{'ArchMinPrefs_constraint_one_per_body'} = 'CREATE UNIQUE INDEX one_per_body ON ArchMinPrefs (server_id, body_id)';
...
CREATE TABLE SitterPasswords (
  ...
  sitter varchar(64)
    ...
    $table_statements->{'SitterPasswords_constraint_one_player_per_server'} = 'CREATE UNIQUE INDEX one_player_per_server ON SitterPasswords (server_id, player_id)';
...
';
    $table_statements->{'LotteryPrefs_body'} = 'CREATE UNIQUE INDEX LotteryPrefs_body ON LotteryPrefs (body_id, server_id)';
